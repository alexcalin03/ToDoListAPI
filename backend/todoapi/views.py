...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def mark_all_as_cleared(request):
    notifications = Notification.objects.filter(recipient=request.user)
    updated_count = notifications.update(status='Cleared')
    return Response({'message': f'{updated_count} notifications marked as Cleared'}, status=status.HTTP_200_OK)
...
def update_password(request):
    ...
    # Ensure both old_password and new_password are provided
